...
if (nsame.gt.0) then
   write(*,*) 'There are levels with the same labels, continue anyway (y/n)?'
   read(*,*) ans
...
!  
   ...
   if  (jparity(nfile,h)(7:7).eq.'-') then 
      latexstring(h) = '$'//trim(labelstring)//'_{'//jparity(nfile,h)(1:5)//'}^o$'
   else
      latexstring(h) = '$'//trim(labelstring)//'_{'//jparity(nfile,h)(1:5)//'}$'
   end if
