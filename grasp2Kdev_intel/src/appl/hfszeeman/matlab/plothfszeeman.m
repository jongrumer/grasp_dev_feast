...
%                J-value                                                     %
%  M(1:2)	educed nuclear matrix elements in a.u., computed in          %
%                nuclear.m                                                   %
...
% Define plots                                       
N_plots = 0;        % Counter for the number of plots.
y = 1;
Y = 1;
n = 0;
N = 0;
moreplots = 1;
moreplots = input('\nWould you like a plot? (Y/N) ');
while moreplots == 1
...
for MF = Fmax:-1:-Fmax
  ...
  for i = 1:ndim1    
    ...
    [dummy,indi] = max(mix);
    fprintf(fp,'%6.0f',indi);  
    fprintf(fp,'%16.9f',Evals(i));
    fprintf(fp,'\n');


    for j = 1:N_Feigvec
      fprintf(fp,'%11.8f ',mix(j));
    end
    fprintf(fp,'\n');
  end
...
display(' ')
display('PLOTHFSZEEMAN finished')
display(' ')
