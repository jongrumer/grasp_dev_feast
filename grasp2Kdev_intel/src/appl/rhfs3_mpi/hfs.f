...
c      print *, ' hfs 1'
      ...
      CALL ALLOC (PNTHFCtmp,5*NVEC*NVEC,8)
*
*   Initialise
*
      DO 2 I = 1,5
...
*
                  IF (KT .EQ. 1) THEN
                    IF ((ITJPOC-ITJPOR) .EQ. 0) THEN
                     HFCtmp(1,NVEC*(K-1)+KK) =   HFCtmp(1,NVEC*(K-1)+KK)
     :                                       + CONTR
                    ELSEIF ((ITJPOC-ITJPOR) .EQ. 2) THEN
                     HFCtmp(2,NVEC*(K-1)+KK) =   HFCtmp(2,NVEC*(K-1)+KK)
     :                                       + CONTR
                    ENDIF
*
*   Electric quadrupole
*
                  ELSEIF (KT .EQ. 2) THEN
                    IF ((ITJPOC-ITJPOR) .EQ. 0) THEN
                     HFCtmp(3,NVEC*(K-1)+KK) =   HFCtmp(3,NVEC*(K-1)+KK)
     :                                       + CONTR
                    ELSEIF ((ITJPOC-ITJPOR) .EQ. 2) THEN
                     HFCtmp(4,NVEC*(K-1)+KK) =   HFCtmp(4,NVEC*(K-1)+KK)
     :                                       + CONTR
                    ELSEIF ((ITJPOC-ITJPOR) .EQ. 4) THEN
                     HFCtmp(5,NVEC*(K-1)+KK) =   HFCtmp(5,NVEC*(K-1)+KK)
     :                                       + CONTR
                    ENDIF
                  ENDIF
    7           CONTINUE
...
*
   ...
   11 CONTINUE
              ...
              CALL MPI_ALLREDUCE (HFCtmp,HFC,5*NVEC*NVEC,
