...
!     NCFD = 2
       ...
       CALL ALLOC (IQA,  NNNWP,  NCFD, 'IQA',   'LODCSL')
       CALL ALLOC (JQSA, NNNWP,3,NCFD, 'JQSA',  'LODCSL')
       CALL ALLOC (JCUPA,NNNWP,  NCFD, 'JCUPA', 'LODCSL')
...
!        where NCF is incremented
         IF (NCF > NCFD) THEN 
            NEWSIZ = NCFD + NCFD/2 
            CALL RALLOC (IQA,  NNNWP,  NEWSIZ, 'IQA',   'LODCSL')
            CALL RALLOC (JQSA, NNNWP,3,NEWSIZ, 'JQSA',  'LODCSL')
            CALL RALLOC (JCUPA,NNNWP,  NEWSIZ, 'JCUPA', 'LODCSL')
            NCFD = NEWSIZ 
         ENDIF 
!
...
!
         DO I = 1, NCORE 
            CALL PACK (NKJ(I) + 1, I, IQA(1:NNNWP,NCF)) 
            CALL PACK (0, I, JQSA(1:NNNWP,1,NCF)) 
            CALL PACK (0, I, JQSA(1:NNNWP,2,NCF)) 
            CALL PACK (1, I, JQSA(1:NNNWP,3,NCF)) 
         END DO 
!
    ...
    9       CONTINUE 
            IF (.NOT.EMPTY .AND. .NOT.FULL) THEN 
               ...
               IF (NOPEN > 1) THEN 
                  ...
                  ENDIF 
   11             CONTINUE 
                  CALL PACK (JXN + 1, NOPEN - 1, JCUPA(1:NNNWP,NCF)) 
                  JLAST = JXN 
               ELSE 
                  JLAST = JSUB 
               ENDIF 
            ENDIF 
            CALL PACK (IOCCI, I, IQA(1:NNNWP,NCF)) 
            CALL PACK (NU, I, JQSA(1:NNNWP,1,NCF)) 
            CALL PACK (0, I, JQSA(1:NNNWP,2,NCF)) 
            CALL PACK (JSUB + 1, I, JQSA(1:NNNWP,3,NCF)) 
         END DO 
!
         DO I = MAX(1,NOPEN), NW 
            CALL PACK (0, I, JCUPA(1:NNNWP,NCF)) 
         END DO 
!
         IF (NQSN /= NQS) THEN 
...
!
         JPI = (JLAST + 1)*IPTY 
         CALL PACK (JPI, NNNW, JCUPA(1:NNNWP,NCF)) 
!
         IF (NCF > 1) THEN 
...
!
         IF (NCF == 1) THEN 
            DO I = 1, NCORE 
               CALL PACK (NKJ(I) + 1, I, IQA(1:NNNWP,1)) 
               CALL PACK (0, I, JQSA(1:NNNWP,1,1)) 
               CALL PACK (0, I, JQSA(1:NNNWP,2,1)) 
               CALL PACK (1, I, JQSA(1:NNNWP,3,1)) 
            END DO 
            CALL PACK (0, 1, JCUPA(1:NNNWP,1)) 
            CALL PACK (1, NNNW, JCUPA(1:NNNWP,1)) 
         ELSE 
            NCF = NCF - 1 
         ENDIF 
!
   ...
   19 CONTINUE 
      IF (I <= NW) THEN 
         ...
         DO II = I, NW 
            ...
            DO J = 1, NCF 
               ITEMP = IQ(II + 1,J) 
               CALL PACK (ITEMP, II, IQA(1:NNNWP,J))
               ITEMP = JQS(1,II + 1,J) 
               CALL PACK (ITEMP, II, JQSA(II:NNNWP,1,J)) 
               ITEMP = JQS(2,II + 1,J) 
               CALL PACK (ITEMP, II, JQSA(II:NNNWP,2,J)) 
               ITEMP = JQS(3,II + 1,J) 
               CALL PACK (ITEMP, II, JQSA(II:NNNWP,3,J)) 
            END DO 
         END DO 
   23    CONTINUE 
         I = I + 1 
