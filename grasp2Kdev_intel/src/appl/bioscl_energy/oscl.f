...
c             M = LEVFF+NVECII*(LEVII-1)
              ...
              ANSW = 'n'
              WRITE(*,*) ' Current transition energy [a.u]  = ', ET(M)
              WRITE(*,*)
10            WRITE(*,*) ' Do you want to change this energy (y/n)?'
              READ(*,'(A1)') ANSW
              IF(ANSW.EQ.'N'.OR.ANSW.EQ.'n'.OR.ANSW.EQ.'Y'.OR.
     :          ANSW.EQ.'y') THEN
                IF(ANSW.EQ.'Y'.OR.ANSW.EQ.'y') THEN
                  WRITE(*,*) ' Give new transition energy [a.u]:'
                  READ(*,'(1D20.7)') ET(M)
                  WRITE(*,*) 'New energy is [a.u] =', ET(M)
                ELSEIF(ANSW.EQ.'N'.OR.ANSW.EQ.'n') THEN
                  WRITE(*,*) 'Transition energy is unchanged.'
                END IF
              ELSE
                WRITE(*,*) 'Please answer in format Y/y/N/n, redo!'
                GOTO 10
              ENDIF
