...
c     POINTER (PNTEMT,EMT(1999))
     ...
     :      /WHERE/IMCDF,NREC
      ...
      integer :: solver, m0, mout, finfo
      integer, dimension(1:63) :: fpm
      integer, allocatable, dimension(:) :: iaf, irowf

      character(len=1) :: uplo

      double precision :: evmin, evmax
      double precision, allocatable, dimension(:) :: evout, resid
      double precision, allocatable, dimension(:) :: emtf
      double precision, allocatable, dimension(:,:) :: evecout
!-----------------------------------------------------------------------
      !PRINT *, 'maneig ...'
! pfeast hack, local nprocss
      nprocss = 1
...
*
     ...
     :            nmv,ierr)
      ...
      elseif(solver == 1 ) then
          ...
          write(*,*) myid,n1000,ncf,niv
!         if(myid==0) write(*,*) iendc(1:ncf)
!         if(myid==0) write(*,*) emt(1:iendc(ncf))
	  CALL iniest3p(n1000, ncf, niv, evmin, evmax, emt, 
     :     iendc, irow)

!         write(*,*) 'ELIMIT',evmin, evmax
!         evmin = evmin - 100.0d0
!         evmax = evmax + 100.0d0
          evmin = evmin - 0.1d0
          evmax = evmax + 0.1d0
...
!         m0 = niv*10
          ...
          iaf(ncf+1) = iendc(ncf)+1

!         write(*,*) uplo,ncf,evmin,evmax,m0
!         write(*,*) iaf
!         write(*,*) '+++++++++++'
!         write(*,*) iendc(1:556+2)
!         write(*,*) irow(1:36074)
...
!         allocate(emtf(iendc(ncf)))
!         allocate(irowf(iendc(ncf)))

!         do j= 1, iendc(ncf)

!            emtf(j) = emt(j)
!            irowf(j) = irow(j)
!            if(myid==0)write(*,*) j,irowf(j), emtf(j)

!         end do

!         do j=1, ncf+1

!           write(*,*) j,iaf(j), iendc(j)

!         end do
!         if(myid==0) then

!          write(300+myid,*) iaf(ncf+1)
!          write(300+myid,*) iendc(1:ncf)
!          write(300+myid,*) iaf(1:ncf+1)
!          write(300+myid,*) irow(1:iaf(ncf+1)-1)
!          write(300+myid,*) emt(1:iaf(ncf+1)-1)
...
!         endif
          ...
          write(*,*) 'CALLING FEAST'
          call dfeast_scsrev(uplo,ncf,emt,iaf,irow,fpm,epsout,nloops,
     :     evmin, evmax, m0, evout, evecout, mout,resid, finfo)
          write(*,*) 'FEAST DONE'
!         write(*,*) 'RESULT',mout,finfo
!         write(*,*) evout

      else

          write(*,*) 'Solver not found'
          stop
...
*
     ...
     &                    evec( nevecpat + ncf*(j-1) + 1)    ,    1)
      ...
      elseif(solver == 1) then
        ...
        if(nevblk(jblock) > mout) then

          write(*,*) 'Too few eigenvalues found',nevblk(jblock),mout
          stop
        ...
        DO j = 1, nevblk(jblock)
          ...
          eval(ncminpat+j) = evout(j)

          CALL dcopy(ncf, evecout(1,j), 1,
     &                    evec( nevecpat + ncf*(j-1) + 1)    ,    1)
!         CALL dcopy(ncf, evecout( ncf*(j-1) + 1 ), 1,
!    &                    evec( nevecpat + ncf*(j-1) + 1)    ,    1)
