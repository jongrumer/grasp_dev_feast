...
!-----------------------------------------------------------------------
     ...
     & PRINT *, 'Analysing sparse matrix array definition file ...', 30
      ...
      inquire(30, opened=ok, named=hasname,name=pname)
      if(ok .and. hasname) write(*,*) 'ROOT30 ',pname, myid
...
* local, and its length is determined by the local parameter nnonz.
      ...
      DO I = 1, NNONZ
*P       write(*,*) 'I=',i
         READ (30,iostat=ios,err=100) IC, IROW(I), npos
