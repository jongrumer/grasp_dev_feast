...
c     POINTER (PNTEMT,EMT(1999))
     ...
     :      /WHERE/IMCDF,NREC
      ...
      integer :: solver, m0, mout, finfo, lagom, m0iter
      integer, dimension(1:63) :: fpm
      integer, allocatable, dimension(:) :: iaf, irowf

      character(len=1) :: uplo

      double precision :: evminorg, evmaxorg, evminny, evmaxny
      double precision :: devorg, devny
...
*
     ...
     :            nmv,ierr)
	  ...
	  CALL iniest3 (8*n1000, ncf, niv, evminorg, evmaxorg, emt, 
     :     iendc, irow)

          write(*,*) 'ELIMIT',evminorg, evmaxorg
!         evmin = evmin - 100.0d0
!         evmax = evmax + 100.0d0
!         evmin = evmin - 0.1d0
!         evmax = evmax + 0.1d0
...
!         write(*,*) fpm
     ...
     :     evminny, evmaxny, m0, evout, evecout, mout,resid, finfo)

          m0iter = m0iter + 1

          if (mout > int(m0) .or. finfo == 3) then
            ...
            write(*,*) 'OLD EV=',evminny,evmaxny

            devorg = evmaxny - evminny
            write(*,*) 'DEVORG=',devorg
            write(*,*) m0,devorg,mout
            devny = 1.1*m0*devorg/mout
            write(*,*) 'DEVNY=',devny
            evmaxny = evminny + devny
            evminny = evminny - 0.1d0*devny

            write(*,*) 'NEW EV=',evminny,evmaxny
