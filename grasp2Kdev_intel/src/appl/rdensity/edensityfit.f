...
!     Fits polynomial b1 + b2r^2 b3r^3 + b4r^4 to (r,rho) electron density data using least squares method
      ...
      DOUBLE PRECISION :: XVEC(NNNP), YVEC(NNNP), RHO, Z, DRMS
      DOUBLE PRECISION :: CDET, AU2FM, W(NNNP), NORM, RES, F(5)
      DOUBLE PRECISION :: PI, CONST, CONST2, A0, A1, A2
      DOUBLE PRECISION :: DR(4), FDSUM, PAR(2), FO90
      DOUBLE PRECISION :: DR2(4),PARF(2),PARF2(2),R2,DR4,DR6
      DOUBLE PRECISION :: DY(NNNP),DF2(3),DE
      INTEGER :: I, NMIN, NMAX, NRNUC, NR, NPARFIT

      PI = 3.14159265358979d0
      AU2FM = 52917.72083d0
      CONST = 27.2113834d0*AU2FM*1000.0d0  ! To get electronic factors in meV
      CONST2 = CONST*241.7989505  ! To get electronic factors in GHz
...
!     DetermINe fIttINg parameters
         ...
         F(1) = 2.0*PI/3.0*Z*P(1)*CONST2
         F(2) = PI/5.0*Z*P(2)*CONST2
         F(3) = 2.0*PI/21.0*Z*P(3)*CONST2
...
!     DetermINe INverse of C matrIx
         ...
         write(*,*)
         write(*,*) 'F(2): ', F(2)
         write(*,*) 'Error in F(2): ',sqrt(CI(1,1))*PI/5.0*Z*
     :        CONST2*NORM
         DF2(1) = CI(1,1)*(PI/5.0*Z*CONST2*NORM)**2.0
         !write(*,*) sqrt(CI(2,2))
         write(*,*) 'F(3): ', F(3)
         write(*,*) 'Error in F(3): ',sqrt(CI(2,2))*2.0*PI/21.0*Z*
     :        CONST2*NORM
         DF2(2) = CI(2,2)*(2.0*PI/21.0*Z*CONST2*NORM)**2.0
         write(*,*) CI(1,1)
         write(*,*) CI(2,2)
         write(*,*) CI(2,1)
         DF2(3) = CI(2,1)*(2.0*PI/21.0*Z*CONST2*NORM)*
     :        (PI/5.0*Z*CONST2*NORM)
         write(*,*) 'DF2(3): ', sqrt(abs(DF2(3)))
...
!     PURELY FOR TESTING PUPOSES
     ...
     :        (PARF2(1)**2.d0 - PARF(1)**2.d0)
         ...
         DO I=1,3 
            ...
            FDSUM = FDSUM + F(I)*DR2(I) 
         END DO
         DE = DR2(2)**2.*DF2(1) + DR2(3)**2.*DF2(2)
         DE = DE + 2.*DR2(2)*DR2(3)*DF2(3)              ! Adding co-variance
         DE = sqrt(DE)
         F(5) = FDSUM
         write(*,*) FDSUM, DE
         write(*,*) 
