...
*
      ...
      DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE :: DENSMAT3             ! JE ADD
...
*
      ...
      POINTER (PNTRPF,PF(NNNP,*)),(PNTRQF,QF(NNNP,*))   ! JE ADD
*
...
!     :                             NP(IA),NH(IA),NP(IB),NH(IB),CONTRI1
 ...
 13   CONTINUE
      ...
      ! Write out orbitals
      K=INDEX(NAME,' ')
      FILNAM = NAME(1:K-1)//'.orb'
      OPEN(36,FILE=FILNAM,FORM='FORMATTED',STATUS = 'UNKNOWN')
      write(36,*) '     r                   P(r)                   Q(r)'
      DO I = 1,NW
         WRITE(36,'(i3,a)') NP(I),NH(I)
         DO J = 1,MF(I)
            WRITE(36,'(3D20.10)') R(J), PF(J,I), QF(J,I)
         END DO
         write(36,*)
      ENDDO
      CLOSE(36)
      ...
      DO IK = MINK,MAXK
         ...
         IF (KFLAG(IK).GT.0) THEN
            ...
            ! ALLOCATE( DENSMAT3(NDIM,NDIM) )

            ! For some reason an error sometimes occur here:
            ! *** glibc detected *** /home/per/programs/grasp/grasp2k_light/bin/rdensity: malloc(): memory corruption: 0x0000000000cace50 *** 
            write(*,*) 
            write(*,*) '-------------------------------------'
     ...
     :                       DENSMAT(IV,IK,IA+MINN-1,IB+MINN-1)
            ...
            DO IA = 1,NDIM
               write(*,'(i4,12f12.7)')IA+MINN-1,DENSMAT2(1:NDIM,IA)
            END DO
            write(*,*)
     ...
     :           NDIM,eig,work,LPAR,inf)
            ...
            write(*,*) 'eigenvectors:'
            DO IA = 1,NDIM
               write(*,'(i4,12f12.7)') IA,DENSMAT3(1:NDIM,IA)
            END DO
            ...
            IF(NDIM.GT.1) THEN
               ...
               DO IA = 1,NDIM
                  CONTRIB = 0.0
                  DO IB = 1,NDIM
                     IF(ABS(DENSMAT3(IB,IA)).GT.CONTRIB) THEN
