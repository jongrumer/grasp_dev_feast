...
*R    PRINT*,' In biotrn: KLPI    = ',KLPI
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLPF = KFREE
      KFREE = KFREE + NLFMX * NGRID
*R    PRINT*,' In biotrn: KLPF    = ',KLPF
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLPIF = KFREE
      KFREE = KFREE + NLIFMX * NGRID
*R    PRINT*,' In biotrn: KLPIF   = ',KLPIF
*R    PRINT*,'            KFREE   = ',KFREE
*
...
*R    PRINT*,' In biotrn: KLSTOT  = ',KLSTOT
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLSIF = KFREE
      KFREE = KFREE + NLIFMX ** 2
*R    PRINT*,' In biotrn: KLSIF   = ',KLSIF
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLSIFI = KFREE
      KFREE = KFREE + NLIFMX ** 2
*R    PRINT*,' In biotrn: KLSIFI  = ',KLSIFI
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLCI = KFREE
      KFREE = KFREE + NLIFMX ** 2
*R    PRINT*,' In biotrn: KLCI    = ',KLCI
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLCF = KFREE
      KFREE = KFREE + NLIFMX ** 2
*R    PRINT*,' In biotrn: KLCF    = ',KLCF
*R    PRINT*,'            KFREE   = ',KFREE
*
      KLSCR = KFREE
      KFREE = KFREE + NLIFMX ** 2 + NLIFMX*(NLIFMX+1)
*R    PRINT*,' In biotrn: KLSCR   = ',KLSCR
*R    PRINT*,'            KFREE   = ',KFREE
*R    PRINT*,'         =>  FREE =     ',KFREe
*
...
*
*R       WRITE(*,*) 'Transformation matrices initial'
*R       CALL WRTMAT(SCR(KLCI),NI,NI,NI,NI)
         CALL MATML4(SCR(KLPIF),SCR(KLPI),SCR(KLCI),
     &               NGRID,NI,NGRID,NI,NI,NI,0)
         CALL COPVEC(SCR(KLPIF),PI(1,ILI),NI*NGRID)
*R       WRITE(*,*) 'Transformation matrices final'
*R       CALL WRTMAT(SCR(KLCF),NF,NF,NF,NF)
