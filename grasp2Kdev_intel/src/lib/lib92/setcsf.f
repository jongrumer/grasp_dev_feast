************************************************************************
*
      SUBROUTINE setcsf (nfile, name, ncore)
*
* The only difference from setcsh is that lodcsf is called instead of
* lodcsh. lodcsf loads all CSFs; lodcsh loads header only (the CSFs
* are loaded by subsequent lodcsh2).
*
*   Open, check the CSL file and load the load (via lodcsl) data from 
*   the header lines. It is designed to replace all kinds of "setcsl" 
*   routines within GRASP packages.
*
*   Routines called: lodcsf
*
