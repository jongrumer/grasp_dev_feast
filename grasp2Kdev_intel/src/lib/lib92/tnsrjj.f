...
*
   99 CONTINUE
CGG
CGG Gediminas NIST 2005.10.13
      IF (NQ1(IA1).EQ.0 .AND. NQ2(IA2).EQ.0) GOTO 100
*
      DO 26 JW = 1,NS
        IJ = JLIST(JW)
...
*
   ...
   24   CONTINUE
        ...
        DO 25 K = 1,3
CGG
CGG Gediminas NIST 2005.10.04
CGG          IF (JBQ1(K,IJ) .NE. JBQ2(K,IJ)) GOTO 40
          IF (JBQ1(K,IJ) .NE. JBQ2(K,IJ)) GOTO 100 
   25   CONTINUE
   26 CONTINUE
*
...
*
      NELCTS = NQ2(IA2)
* PER **
      if ((nelcts .le. 0) .or. (nelcts .gt. ks2)) then
         write(*,*) 'IN TNSRJJ 1,JA,JB',JA,JB
         pause
         return
      end if  
...
*
        NELCTS = NQ1(IA1)
* PER **
      if ((nelcts .le. 0) .or. (nelcts .gt. ks1)) then
         write(*,*) 'IN TNSRJJ 2,JA,JB',JA,JB
         return
      end if  
